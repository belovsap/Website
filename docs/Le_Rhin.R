...
g_scores = get_nrc_sentiment(le_rhin)
class(g_scores)
g_scores
...
r_text_sen = get_sentences(le_rhin)
r_text_sen

sentiment_vector = get_sentiment(r_text_sen, method = "syuzhet")
...
sentence_sentiment = data.frame(r_text_sen, sentiment_vector)
...
wordcloud(le_rhin, min.freq=3,max.words=25,colors=c("blue","green"))
